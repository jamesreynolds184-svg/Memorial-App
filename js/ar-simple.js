(() => {
  ...
  if (customText) msgEl.textContent = customText;
  ...
  let currentHeading = 0;         // device yaw (alpha)
  let anchorHeading = null;       // world direction anchor is fixed at
  const FOV = 80;                 // visible half-angle (±FOV shows on screen)
  const HIDE_MARGIN = 5;          // deg extra before hide
  let orientationReady = false;
  ...
  function relock(){
    if (anchorHeading != null) {
      ...
      anchorEl.textContent = 'Test Anchor (re‑locked)';
    }
  }
    ...
    if (window.DeviceOrientationEvent?.requestPermission) {
      DeviceOrientationEvent.requestPermission()
        .then(res=>{
          ...
          else hint.textContent = 'Motion denied';
        })
        .catch(()=> hint.textContent = 'Motion error');
    } else {
      addListeners();
    }
  }
  function addListeners(){
    window.addEventListener('deviceorientation', e=>{
      ...
      if (anchorHeading == null) {
        anchorHeading = currentHeading; // initial lock straight ahead
        anchorEl.textContent = 'Test Anchor (locked)';
        hint.textContent = 'Rotate device to move anchor off screen';
      }
    }, true);
  }
  ...
  function loop(){
    if (anchorHeading != null){
      ...
      let rel = ((anchorHeading - currentHeading + 540) % 360) - 180;
      // Within field of view?
      if (Math.abs(rel) <= (FOV + HIDE_MARGIN)) {
        const w = window.innerWidth;
        // Map rel (−FOV..FOV) to screen (0..w)
        const x = (rel / FOV) * (w/2) + w/2;
      ...
      // (Optional) subtle scale: as it nears edge shrink a bit
      const edgeFactor = Math.min(1, Math.abs(rel)/FOV);
